...
class MainActivity : AppCompatActivity() {
    ...
    private fun showLogDialog(logs: String) {
        val dialog = AlertDialog.Builder(this).setTitle("Log").setMessage(logs).
                setPositiveButton("Extract logs to file", { _,_ -> extractLogs() } ).create()
        dialog.show()
        val textView = dialog.findViewById<TextView>(android.R.id.message)
        textView.textSize = 12f
    }

    private fun extractLogs(){
        if(ContextCompat.checkSelfPermission(this, Manifest.permission.WRITE_EXTERNAL_STORAGE)
            ...
            val file = logController.extractLogs()
            if(file != null){
                toast("Logs extracted to ${file.absolutePath}")
            } else {
                toast("Logs extraction failed")
            }
        }
    }
    ...
    override fun onRequestPermissionsResult(requestCode: Int, permissions: Array<out String>, grantResults: IntArray) {
        ...
        if(requestCode == PERMISSION_REQUEST_WRITE_EXTERNAL_STORAGE && grantResults[0] == PackageManager.PERMISSION_GRANTED){
            ...
            toast("Writing permission required to extract logs")
        }
    }
}
